...
Similar to Jupyter, RStudio now also supports a notebook interface (see [link](https://bookdown.org/yihui/rmarkdown/notebook.html) for details). Code chunks
...
on Wikipedia). `Markdown` documents are pure text, but can be converted into various
output formats (HTML, PDF and Word in RStudio).

In addition, `Rmarkdown` documents can contain code chunks that will be run
upon conversion, and their output (text or figures) can be captured and integrated
...
automatically convert R `dgcMatrix` sparse matrix objects to [SciPy CSC sparse
...
```{python, class.source="pythonchunk"}
# automatic conversionto SciPy CSC matrix (requires scipy)
r.A
r.A.shape
```
